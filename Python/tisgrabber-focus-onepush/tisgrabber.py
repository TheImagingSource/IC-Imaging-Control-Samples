...
#	@sa IC_CloseLibrary
    InitLibrary = __tisgrabber.IC_InitLibrary
    InitLibrary.restype = C.c_int
    InitLibrary.argtypes = (C.c_char_p,)
...
# ############################################################################
    CloseVideoCaptureDevice = __tisgrabber.IC_CloseVideoCaptureDevice
    CloseVideoCaptureDevice.restype = GrabberHandlePtr     
    CloseVideoCaptureDevice.argtypes = (GrabberHandlePtr,)

# ############################################################################
    ReleaseGrabber = __tisgrabber.IC_ReleaseGrabber
    ReleaseGrabber.restype = GrabberHandlePtr     
    ReleaseGrabber.argtypes = (C.POINTER(GrabberHandlePtr),)

# ############################################################################
    CloseLibrary = __tisgrabber.IC_CloseLibrary
    CloseLibrary.restype = C.c_int
    CloseLibrary.argtypes = None
...
class TIS_CAM(object):
        ...
        def ReleaseGrabber(self):
            TIS_GrabberDLL.ReleaseGrabber( self._handle)
        ...
        def PropertyAvailable(self, Property, Element):
            Null = None
        ...
        def SaveImage(self,FileName, FileType, Quality=75):
            ...
            :return: Error code
            '''
            return TIS_GrabberDLL.SaveImage(self._handle, self.s(FileName), ImageFileTypes[FileType],Quality)
